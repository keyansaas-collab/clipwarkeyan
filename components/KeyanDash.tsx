...
function SetterReport({ setter, biz, onBack }: { setter: SetterRow; biz: BizParams; onBack: () => void }) {
  ...
  const [active, setActive] = useState(setter.is_active);
  const [togBusy, setTogBusy] = useState(false);

  async function toggleActive() {
    const next = !active;
    if (next === false && !window.confirm(`Retirer ${setter.name.split(" ")[0]} de l'équipe ?\n\nIl ne pourra plus se connecter à SetWar. Son historique et ses ventes sont conservés. Tu peux le réactiver à tout moment.`)) return;
    setTogBusy(true);
    try {
      await db.rpc("set_user_active", { p_user: setter.setter_id, p_active: next });
      setActive(next);
    } catch {
      /* noop */
    } finally {
        ...
        <header className="kd-head">
          ...
          <div className="kd-rhead">
            ...
            <div style={{ flex: 1 }}>
              ...
              <p className="kd-sub">Rapport individuel {!active && "· retiré de l'équipe"}</p>
            </div>
            <button className={"kd-toggle" + (active ? "" : " off")} disabled={togBusy} onClick={toggleActive}>
              {togBusy ? "…" : active ? "Retirer de l'équipe" : "Réactiver"}
            </button>
          </div>
        </header>
...
function KeyanStyle() {
  return (
    ...
    .kd-toggle{flex:none;background:none;border:1px solid var(--line2);border-radius:12px;padding:10px 16px;font-family:inherit;font-weight:700;font-size:13px;color:var(--hot);cursor:pointer;transition:background .15s}
    .kd-toggle:hover{background:var(--hot-dim)}
    .kd-toggle.off{color:var(--lime);border-color:rgba(58,107,46,.3)}
    .kd-toggle.off:hover{background:var(--lime-dim)}
    /* KPIs */
