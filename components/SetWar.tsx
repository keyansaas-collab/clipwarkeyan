...
export default function SetWar({ userName }: { userName?: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [p, s, cn] = await Promise.all([db.rpc("setter_prospects"), db.rpc("setter_stats"), db.rpc("my_coach_note")]);
    setRows((p.data as Prospect[]) || []);
    setStats(s.data || null);
    setCoachNote(typeof cn.data === "string" ? cn.data : ((cn.data as any)?.note || null));
    setLoading(false);
  }, [db]);
    ...
    <div className="sw-root">
      ...
      {screen === "today" && (
        <div className="sw-screen">
          ...
          {!loading && coachNote && (
            <div className="sw-coachbanner">
              <span className="sw-cbicon">💬</span>
              <div><div className="sw-cbtitle">Mot de Keyan</div><div className="sw-cbtext">{coachNote}</div></div>
            </div>
          )}
  ...
  return (
    <div className="sw-fiche">
      ...
      <div className="sw-fbody">
        {p.coach_note && (
          <div className="sw-keyannote"><span>💬 Keyan :</span> {p.coach_note}</div>
        )}
...
function Profil({ first, userName, stats, rows, onFlash }: { first: string; userName?: string; stats: any; rows: Prospect[]; onFlash: (m: string) => void }) {
  ...
  // charge l'avatar + le nom réels du profil
  useEffect(() => {
    (async () => {
      const { data: sess } = await db.auth.getSession();
      const uid = sess.session?.user.id;
      if (!uid) return;
      const { data } = await db.from("profiles").select("display_name, avatar_url").eq("id", uid).single();
      if (data) {
        setAvatarUrl((data as any).avatar_url || null);
        if ((data as any).display_name) setName((data as any).display_name);
      }
    })();
  }, [db]);
  ...
  async function uploadAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { onFlash("Photo trop lourde (max 5 Mo)"); return; }
    setUploading(true);
    const { data: sess } = await db.auth.getSession();
    const uid = sess.session?.user.id;
    ...
    const { error } = await db.storage.from("avatars").upload(path, file, { upsert: true, cacheControl: "3600" });
    if (error) { setUploading(false); onFlash("Échec de l'upload"); return; }
    const url = db.storage.from("avatars").getPublicUrl(path).data.publicUrl + "?t=" + Date.now();
    await db.from("profiles").update({ avatar_url: url }).eq("id", uid);
    setAvatarUrl(url);
    setUploading(false);
    onFlash("Photo mise à jour ✨");
  }

  async function saveName() {
    const clean = name.trim();
    if (!clean) { setEditName(false); return; }
    setSavingName(true);
    const { data: sess } = await db.auth.getSession();
    const uid = sess.session?.user.id;
    if (uid) await db.from("profiles").update({ display_name: clean }).eq("id", uid);
    setSavingName(false);
    setEditName(false);
    onFlash("Nom mis à jour ✓");
  }
  ...
  const displayName = name || userName || first;
    ...
    <div className="sw-screen">
      <header className="sw-head">
        ...
        <div className="sw-pidentity">
          <button className="sw-avaedit" onClick={() => fileRef.current?.click()} aria-label="Changer la photo">
            {avatarUrl ? (
              <img src={avatarUrl} alt="" className="sw-avaimg" />
            ) : (
              <div className="sw-ava hot" style={{ width: 64, height: 64, fontSize: 26, borderRadius: 20 }}>{initialOf(displayName)}</div>
            )}
            <span className="sw-avacam">{uploading ? "…" : "📷"}</span>
          </button>
          <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={uploadAvatar} />
          <div style={{ flex: 1, minWidth: 0 }}>
            {editName ? (
              <div className="sw-nameedit">
                <input className="sw-nameinput" value={name} autoFocus autoCapitalize="words"
                  onChange={(e) => setName(e.target.value)} placeholder="Ton nom" />
                <button className="sw-namesave" disabled={savingName} onClick={saveName}>{savingName ? "…" : "OK"}</button>
              </div>
            ) : (
              <div className="sw-pname" onClick={() => setEditName(true)}>
                {displayName} <span className="sw-editpen">✎</span>
              </div>
            )}
...
function SetWarStyle() {
  return (
    ...
    .sw-coachbanner{margin:14px 22px 0;display:flex;gap:12px;align-items:flex-start;background:var(--blue-dim);border:1px solid rgba(47,111,214,.2);border-radius:16px;padding:14px 16px}
    .sw-cbicon{font-size:18px;flex:none}
    .sw-cbtitle{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:#2A5599;margin-bottom:3px}
    .sw-cbtext{font-size:13.5px;color:#1E3F70;line-height:1.5;font-weight:500}
    .sw-keyannote{margin-bottom:14px;background:var(--blue-dim);border:1px solid rgba(47,111,214,.25);border-radius:14px;padding:13px 15px;font-size:13.5px;color:#1E3F70;line-height:1.5}
    .sw-keyannote span{font-weight:800;color:#2A5599}
    ...
    .sw-avaedit{position:relative;border:none;background:none;padding:0;cursor:pointer;flex:none}
    .sw-avaimg{width:64px;height:64px;border-radius:20px;object-fit:cover;border:1px solid var(--line2)}
    .sw-avacam{position:absolute;bottom:-4px;right:-4px;width:24px;height:24px;border-radius:8px;background:var(--lime);color:#fff;font-size:11px;display:flex;align-items:center;justify-content:center;border:2px solid var(--bg)}
    .sw-pname{font-size:22px;font-weight:800;letter-spacing:-.02em;cursor:pointer;display:flex;align-items:center;gap:8px}
    .sw-editpen{font-size:13px;color:var(--mut2)}
    .sw-nameedit{display:flex;gap:8px;align-items:center}
    .sw-nameinput{flex:1;min-width:0;background:var(--card);border:1px solid var(--line2);border-radius:11px;padding:9px 12px;color:var(--txt);font-family:inherit;font-size:18px;font-weight:700;outline:none}
    .sw-nameinput:focus{border-color:var(--lime)}
    .sw-namesave{flex:none;background:var(--lime);color:var(--lime-ink);border:none;border-radius:11px;padding:9px 16px;font-family:inherit;font-weight:800;font-size:14px;cursor:pointer}
    .sw-prole{font-size:13px;color:var(--mut);margin-top:5px;font-weight:500}
